...
foreach my $call (split(/,/, $calls)) {
    ...
    my $count = 0;
    foreach my $board (@{$mapping->{'boards'}}) {
        unless($board->{'discard'}) {
            $count++;
        }
    }
    ...
    foreach my $row (@{$mapping->{$call}->{'rows'}}) {
        ...
        if($row->{'seen'} == 5) {
            print "Found winner: discarding board\n";
            $row->{'board'}->{'discard'} = 1;
        }
    }


    print "$count boards remain\n";
}